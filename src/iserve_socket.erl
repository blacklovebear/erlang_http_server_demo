...
-define (forbidden_403, "HTTP/1.1 403 Frobidden\r\n\r\n").
-define (not_found_404, "HTTP/1.1 404 Not Found\r\n\r\n").

-record (c, {sock, port, peer_addr, peer_port}).
...
init({ListenPid, ListenSocket, ListenPort}) ->
	...
	{ok, Socket} ->
		...
		iserve_server:create(ListenPid, self()),
		{ok, {Addr, Port}} = inet:peername(Socket),
			C = #c{sock = Socket, port = ListenPort, 
						peer_addr = Addr, peer_port = Port},
...
call_mfa(Path, Data, C, Req) ->
	case iserve:lookup(C#c.port, Req#req.method, Path) of
		{ok, Mod, Func} ->
			case catch Mod:Func(Req, Data) of
				{'EXIT', Reason} ->
